...
func Close() error {
	...
	if _, err := db.Exec("PRAGMA wal_checkpoint(TRUNCATE)"); err != nil {
		infra.LogWarn(fmt.Sprintf("关闭前 WAL checkpoint 失败: %v", err))
	}
	return db.Close()
}
...
// ExecCtx 带 context 的单条写入
func ExecCtx(ctx context.Context, query string, args ...any) error {
	db, err := Handle()
	if err != nil {
		return err
	}
...
// ExecInImmediateTx 在 BEGIN IMMEDIATE 事务中执行"读-改-写"逻辑。
//
...
func ExecInImmediateTx(ctx context.Context, fn func(tx TxExecutor) error) error {
	db, err := Handle()
	if err != nil {
		return err
	}
...
// ExecInTx 在事务中执行一段逻辑，出错自动回滚。
// 用于需要"读-改-写"或跨表一致性的场景。
func ExecInTx(ctx context.Context, fn func(tx *sql.Tx) error) error {
	db, err := Handle()
	if err != nil {
		return err
	}

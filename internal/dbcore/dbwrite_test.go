	...
	for w := 0; w < writers; w++ {
		...
		go func(w int) {
			...
			for i := 0; i < perWriter; i++ {
				if err := Exec(`INSERT INTO wt (v) VALUES (?)`, fmt.Sprintf("w%d-%d", w, i)); err != nil {
					errs <- err
				}
			}
		}(w)
...
func TestDBExecStatementsIsAtomic(t *testing.T) {
	...
	// 全部合法：应全部落库
	err := ExecStatements(ctx, []Statement{
...
func TestDBExecStatementsEmptyIsNoop(t *testing.T) {
	initWriteTestDB(t)
	if err := ExecStatements(context.Background(), nil); err != nil {
...
func TestDBExecSurfacesErrors(t *testing.T) {
	...
	if err := Exec(`INSERT INTO wt (v) VALUES (NULL)`); err == nil {
		t.Error("违反约束的写入必须返回错误")
	}
	if err := Exec(`INSERT INTO no_such_table (v) VALUES (?)`, "x"); err == nil {
...
func TestDBExecCtxRespectsCancellation(t *testing.T) {
	...
	if err := ExecCtx(ctx, `INSERT INTO wt (v) VALUES (?)`, "x"); err == nil {
...
func TestDBExecInTxRollsBackOnError(t *testing.T) {
	...
	err := ExecInTx(ctx, func(tx *sql.Tx) error {
	...
	// 闭包成功时应提交
	if err := ExecInTx(ctx, func(tx *sql.Tx) error {

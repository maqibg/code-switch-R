...
func TestAnthropicStreamToolsUseIncrementalMatrix(t *testing.T) {
	...
	execution, err := service.newRelayForwardExecution(
		...
		services.Provider{Name: "chat", UpstreamProtocol: "openai_chat"},
...
func TestProtocolMatrixRejectsReasoningThatCannotRoundTrip(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if _, err := convertProtocolRequest([]byte(test.body), test.source, test.target); !errors.Is(err, services.ErrClientRequestRejected) {
...
func TestCodexResponsesToChatExecutionRejectsReasoning(t *testing.T) {
	...
	if !errors.Is(err, services.ErrClientRequestRejected) {
...
func TestResponsesCompactRequiresNativeResponsesUpstream(t *testing.T) {
	...
	execution, err := service.newRelayForwardExecution(
		"codex", relayprotocol.OpenAIResponses,
		services.Provider{Name: "responses", UpstreamProtocol: "openai_responses"},
		"/v1/responses/compact", []byte(`{"model":"m","input":"hi"}`), false, "m",
	)
...
func TestPiExecutionNeverEnablesCodexReasoningContinue(t *testing.T) {
	...
	execution, err := service.newRelayForwardExecution(
		...
		services.Provider{Name: "responses", UpstreamProtocol: "openai_responses", CodexReasoningContinueEnabled: true},

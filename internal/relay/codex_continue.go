...
func nextCodexContinueTraceID(provider services.Provider) string {
...
func codexContinueLogEnabled(provider services.Provider) bool {
...
func shouldUseCodexContinue(provider services.Provider, endpoint string, bodyBytes []byte, isStream bool) bool {
	if !provider.CodexReasoningContinueEnabled || !isStream {
		return false
	}
	if provider.ResolveUpstreamProtocol(endpoint) == services.UpstreamProtocolOpenAIChat {
		return false
	}

...
func codexResponsesInputToChatMessages(body map[string]any) ([]map[string]any, error) {
	...
	input, ok := body["input"]
	if !ok || input == nil {
		return nil, services.NewClientRequestRejectedError("Codex Responses 请求缺少 input")
	}
	...
	default:
		return nil, services.NewClientRequestRejectedError("Codex Responses input 必须是 string 或数组")
	}
	if len(messages) == 0 {
		return nil, services.NewClientRequestRejectedError("Codex Responses input 未产生可转发的文本消息")
	}
	return messages, nil
}
...
func codexInputArrayToChatMessages(items []any) ([]map[string]any, error) {
	...
	for index, item := range items {
		...
		if !ok {
			return nil, services.NewClientRequestRejectedError(fmt.Sprintf("input[%d] 必须是对象", index))
		}
		...
		if itemType != "" && itemType != "message" {
			return nil, services.NewClientRequestRejectedError(fmt.Sprintf("Codex Responses input[%d].type=%q 暂不支持", index, itemType))
		}
		role := normalizeCodexRole(stringFromMap(obj, "role"))
		if role == "" {
			return nil, services.NewClientRequestRejectedError(fmt.Sprintf("Codex Responses input[%d].role 缺失或不支持", index))
		}
...
func codexFunctionCallToChatMessage(obj map[string]any, index int) (map[string]any, error) {
	...
	if strings.TrimSpace(callID) == "" {
		return nil, services.NewClientRequestRejectedError(fmt.Sprintf("input[%d].call_id 不能为空", index))
	}
	name := stringFromMap(obj, "name")
	if strings.TrimSpace(name) == "" {
		return nil, services.NewClientRequestRejectedError(fmt.Sprintf("input[%d].name 不能为空", index))
	}
...
func codexFunctionCallOutputToChatMessage(obj map[string]any, index int) (map[string]any, error) {
	...
	if !ok {
		return nil, services.NewClientRequestRejectedError(fmt.Sprintf("input[%d].output 必须是 string", index))
	}
...
func codexTextContent(content any) (string, error) {
	...
	if !ok {
		return "", services.NewClientRequestRejectedError("content 必须是 string 或 text block 数组")
	}
	parts := make([]string, 0, len(items))
	for index, item := range items {
		obj, ok := item.(map[string]any)
		if !ok {
			return "", services.NewClientRequestRejectedError(fmt.Sprintf("content[%d] 必须是对象", index))
		}
		...
		default:
			return "", services.NewClientRequestRejectedError(fmt.Sprintf("content[%d].type=%q 暂不支持", index, blockType))
		}
	}

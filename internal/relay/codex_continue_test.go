...
func TestShouldUseCodexContinueGate(t *testing.T) {
	...
	provider := services.Provider{CodexReasoningContinueEnabled: true}
...
func TestCodexContinueLogEnabledDefaults(t *testing.T) {
	if !codexContinueLogEnabled(services.Provider{}) {
		t.Fatal("expected missing log setting to keep console logs enabled")
	}
	disabled := false
	if codexContinueLogEnabled(services.Provider{CodexReasoningContinueLogEnabled: &disabled}) {
		t.Fatal("expected explicit false to disable console logs")
	}
	enabled := true
	if !codexContinueLogEnabled(services.Provider{CodexReasoningContinueLogEnabled: &enabled}) {

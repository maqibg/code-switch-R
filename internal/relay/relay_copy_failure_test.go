...
func TestUpstreamStreamInterruptionCountsAsProviderFailure(t *testing.T) {
	...
	for _, written := range []bool{false, true} {
		...
		success, err := prs.judgeResponseCopyFailure(c, services.Provider{Name: "BadUpstream"}, execution, upstreamErr)
...
func TestClientWriteFailureDoesNotCountAsProviderFailure(t *testing.T) {
	...
	success, err := prs.judgeResponseCopyFailure(c, services.Provider{Name: "GoodUpstream"}, execution, clientErr)
...
func TestTransformFailureCountsAsProviderFailure(t *testing.T) {
	...
	success, err := prs.judgeResponseCopyFailure(c, services.Provider{Name: "P"}, execution, errors.New("不支持的事件类型"))
	if success {
...
func TestClientAbortTakesPrecedence(t *testing.T) {
	...
	success, err := prs.judgeResponseCopyFailure(c, services.Provider{Name: "P"}, execution, errors.New("any"))
	if success {
		t.Error("不应返回成功")
	}
	if !errors.Is(err, errClientAbort) {

...
func TestTelemetryStatementsIncludeProviderID(t *testing.T) {
	requestStmt := services.RequestLogInsertStatement(services.RequestLog{
	...
	attemptStmt := services.RelayAttemptInsertStatement(telemetry.RequestID, telemetry.Platform, telemetry.SourceID, services.RelayAttemptLog{
...
func TestTelemetryStatementPlaceholdersMatchArgs(t *testing.T) {
	cases := map[string]dbcore.Statement{
		"request_log":   services.RequestLogInsertStatement(services.RequestLog{RequestID: "r"}),
		"relay_attempt": services.RelayAttemptInsertStatement("r", "", "", services.RelayAttemptLog{}),
	}
...
func TestNullableProviderIDMapsZeroToNull(t *testing.T) {
	if got := dbcore.NullableID(0); got != nil {
		t.Errorf("ID 为 0 应写 NULL，实际 %v", got)
	}
	if got := dbcore.NullableID(7); got != int64(7) {
...
func TestTelemetryWritesProviderIDToDatabase(t *testing.T) {
	...
	stmt := services.RequestLogInsertStatement(services.RequestLog{
	...
	// ID 为 0 的情况应落成 NULL
	zeroStmt := services.RequestLogInsertStatement(services.RequestLog{

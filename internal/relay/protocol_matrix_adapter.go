...
func requestToCanonicalChatWithHistory(body []byte, source relayprotocol.Protocol, history []map[string]any, captureMessages bool) (map[string]any, []map[string]any, error) {
	...
	case relayprotocol.GeminiNative:
		...
		return chat, nil, err
	default:
		return nil, nil, services.NewClientRequestRejectedError(fmt.Sprintf("不支持的源协议: %s", source))
	}
}
...
func validateCrossProtocolReasoning(body []byte, source relayprotocol.Protocol) error {
	if !gjson.ValidBytes(body) {
		return services.NewClientRequestRejectedError("协议请求不是合法 JSON 对象")
	}
	switch source {
	case relayprotocol.AnthropicMessages:
		if gjson.GetBytes(body, "thinking").Exists() {
			return services.NewClientRequestRejectedError("跨协议转换暂不支持 Anthropic thinking；请使用原生 Anthropic 上游")
		}
		for _, message := range gjson.GetBytes(body, "messages").Array() {
			for _, block := range message.Get("content").Array() {
				blockType := block.Get("type").String()
				if blockType == "thinking" || blockType == "redacted_thinking" {
					return services.NewClientRequestRejectedError("跨协议转换不能保留 Anthropic thinking 内容块")
				}
			}
		}
	case relayprotocol.OpenAIResponses:
		if gjson.GetBytes(body, "reasoning").Exists() {
			return services.NewClientRequestRejectedError("跨协议转换暂不支持 Responses reasoning；请使用原生 Responses 上游")
		}
		for _, item := range gjson.GetBytes(body, "input").Array() {
			if item.Get("type").String() == "reasoning" {
				return services.NewClientRequestRejectedError("跨协议转换不能保留 Responses reasoning 输入项")
			}
		}
	case relayprotocol.OpenAIChat:
		if gjson.GetBytes(body, "reasoning_effort").Exists() {
			return services.NewClientRequestRejectedError("跨协议转换暂不支持 Chat reasoning_effort；请使用原生 Chat 上游")
		}
		for _, message := range gjson.GetBytes(body, "messages").Array() {
			if message.Get("reasoning_content").Exists() {
				return services.NewClientRequestRejectedError("跨协议转换不能保留 Chat reasoning_content")
			}
		}
	}
	return nil
...
func anthropicRequestToChat(body []byte) (map[string]any, error) {
	...
	if system, exists := source["system"]; exists {
		...
		if err != nil {
			return nil, services.NewClientRequestRejectedError("Anthropic system 仅支持文本内容")
		}
		if text != "" {
	...
	for _, raw := range sourceMessages {
		...
		if !ok {
			return nil, services.NewClientRequestRejectedError("Anthropic messages 项必须是对象")
		}
	...
	if tools, ok := source["tools"].([]any); ok {
		...
		for _, raw := range tools {
			...
			if !ok || stringFromMap(tool, "name") == "" {
				return nil, services.NewClientRequestRejectedError("Anthropic tool 缺少 name")
			}
...
func anthropicMessageToChat(message map[string]any) ([]any, error) {
	...
	if role != "user" && role != "assistant" {
		return nil, services.NewClientRequestRejectedError("Anthropic message role 仅支持 user/assistant")
	}
	...
	if !ok {
		return nil, services.NewClientRequestRejectedError("Anthropic message content 必须是字符串或数组")
	}
	...
	for _, raw := range blocks {
		...
		case "tool_result":
			...
			if err != nil {
				return nil, services.NewClientRequestRejectedError("Anthropic tool_result 仅支持文本内容")
			}
			toolResults = append(toolResults, map[string]any{
				"role": "tool", "tool_call_id": block["tool_use_id"], "content": content,
			})
		default:
			return nil, services.NewClientRequestRejectedError(fmt.Sprintf("暂不支持 Anthropic content block: %s", stringFromMap(block, "type")))
		}
	}
...
func chatRequestToAnthropic(chat map[string]any) ([]byte, error) {
	...
	for _, raw := range sourceMessages {
		message, ok := raw.(map[string]any)
		if !ok {
			return nil, services.NewClientRequestRejectedError("OpenAI Chat messages 项必须是对象")
		}
		role := stringFromMap(message, "role")
		switch role {
		case "system", "developer":
			...
			if err != nil {
				return nil, services.NewClientRequestRejectedError("system/developer message 仅支持文本")
			}
			systemParts = append(systemParts, text)
		case "tool":
			text, err := protocolTextContent(message["content"])
			if err != nil {
				return nil, services.NewClientRequestRejectedError("tool message 仅支持文本")
			}
		...
		case "user", "assistant":
			...
			text, err := protocolTextContent(message["content"])
			if err != nil {
				return nil, services.NewClientRequestRejectedError("Chat message 仅支持文本内容")
			}
			if text != "" {
...
func chatRequestToResponses(chat map[string]any) ([]byte, error) {
	...
	for _, raw := range messages {
		message, ok := raw.(map[string]any)
		if !ok {
			return nil, services.NewClientRequestRejectedError("Chat messages 项必须是对象")
		}
		role := stringFromMap(message, "role")
		if role == "tool" {
			text, err := protocolTextContent(message["content"])
...
func chatToolChoiceToResponses(value any) (any, error) {
	...
	case string:
		...
		case "auto", "none", "required":
			return choice, nil
		default:
			return nil, services.NewClientRequestRejectedError(fmt.Sprintf("tool_choice=%q 暂不支持", typed))
		}
	case map[string]any:
		if stringFromMap(typed, "type") != "function" {
			return nil, services.NewClientRequestRejectedError(fmt.Sprintf("tool_choice.type=%q 暂不支持", stringFromMap(typed, "type")))
		}
		...
		if name == "" {
			return nil, services.NewClientRequestRejectedError("tool_choice function name 不能为空")
		}
...
func (prs *ProviderRelayService) writeBufferedMatrixResponse(c *gin.Context, resp *xrequest.Response, execution *relayForwardExecution, requestLog *services.RequestLog) error {

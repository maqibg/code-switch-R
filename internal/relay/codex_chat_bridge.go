...
func applyCodexChatBridgeOptions(out map[string]any, body map[string]any, history []map[string]any) error {
	if previousID := codexPreviousResponseID(body); previousID != "" && len(history) == 0 {
		return services.NewClientRequestRejectedError("Codex Responses -> Chat bridge 未找到 previous_response_id 对应的历史")
	}
...
func codexChatBridgeHook(converter *CodexChatSSEConverter, requestLog *services.RequestLog) func(data []byte) (bool, []byte) {
...
func (prs *ProviderRelayService) writeCodexChatBridgeResponse(c *gin.Context, resp *xrequest.Response, requestLog *services.RequestLog, chatMessages []map[string]any) error {

...
func TestModelsHandler(t *testing.T) {
	...
	testProvider := services.Provider{
		ID:      1,
		Name:    "TestProvider",
	...
	// 保存 provider 配置
	err := providerService.SaveProviders("claude", []services.Provider{testProvider})
	if err != nil {
...
func TestCustomModelsHandler(t *testing.T) {
	...
	toolId := "mytool"
	kind := "custom:" + toolId
	err := providerService.SaveProviders(kind, []services.Provider{testProvider})
	if err != nil {
		t.Fatalf("保存 provider 配置失败: %v", err)
	}
...
func TestModelsHandler_NoProviders(t *testing.T) {
	...
	providerService := services.NewProviderService()
	if err := providerService.SaveProviders("claude", []services.Provider{}); err != nil {

...
func TestCodexOpenAIChatProtocolNonStreamBridge(t *testing.T) {
	...
	router := newCodexRouteTestRouterWithProvider(t, func(w http.ResponseWriter, r *http.Request) {
		called = true
		if r.URL.Path != httpx.DefaultOpenAIChatEndpoint {
			t.Fatalf("Codex openai_chat 上游路径期望 %s，实际 %s", httpx.DefaultOpenAIChatEndpoint, r.URL.Path)
		}
...
func TestCodexOpenAIChatProtocolStreamBridge(t *testing.T) {
	...
	router := newCodexRouteTestRouterWithProvider(t, func(w http.ResponseWriter, r *http.Request) {
		called = true
		if r.URL.Path != httpx.DefaultOpenAIChatEndpoint {
			t.Fatalf("Codex openai_chat stream 上游路径期望 %s，实际 %s", httpx.DefaultOpenAIChatEndpoint, r.URL.Path)
		}
		var request map[string]any
		if err := json.NewDecoder(r.Body).Decode(&request); err != nil {
			t.Fatalf("解析上游请求失败: %v", err)
...
func TestCodexOpenAIChatProtocolPreservesProviderAPIEndpoint(t *testing.T) {
	...
	}, services.Provider{UpstreamProtocol: string(services.UpstreamProtocolOpenAIChat), APIEndpoint: "/v1/chat/completions"})
...
func TestClaudeOpenAIChatProtocolStreamBridgeUsesUnifiedEndpoint(t *testing.T) {
	...
	router := newProviderRouteTestRouterWithProvider(t, "claude", func(w http.ResponseWriter, r *http.Request) {
		called = true
		if r.URL.Path != httpx.DefaultOpenAIChatEndpoint {
			t.Fatalf("Claude openai_chat 上游路径期望 %s，实际 %s", httpx.DefaultOpenAIChatEndpoint, r.URL.Path)
		}
		var request map[string]any
		if err := json.NewDecoder(r.Body).Decode(&request); err != nil {
			t.Fatalf("解析上游请求失败: %v", err)
...
func TestClaudeOpenAIChatProtocolPreservesProviderAPIEndpoint(t *testing.T) {
	...
	}, services.Provider{UpstreamProtocol: string(services.UpstreamProtocolOpenAIChat), APIEndpoint: "/proxy/chat/completions"})
...
func TestCustomOpenAIChatProtocolStreamBridgeUsesUnifiedExecution(t *testing.T) {
	...
	router := newProviderRouteTestRouterWithProvider(t, "custom:demo", func(w http.ResponseWriter, r *http.Request) {
		called = true
		if r.URL.Path != httpx.DefaultOpenAIChatEndpoint {
			t.Fatalf("Custom openai_chat 上游路径期望 %s，实际 %s", httpx.DefaultOpenAIChatEndpoint, r.URL.Path)
		}
		var request map[string]any
		if err := json.NewDecoder(r.Body).Decode(&request); err != nil {
			t.Fatalf("解析上游请求失败: %v", err)
...
func TestCustomOpenAIChatProtocolPreservesProviderAPIEndpoint(t *testing.T) {
	...
	}, services.Provider{UpstreamProtocol: string(services.UpstreamProtocolOpenAIChat), APIEndpoint: "/custom-openai/chat/completions"})
...
func TestDetectUpstreamProtocolResponses(t *testing.T) {
	if got := services.DetectUpstreamProtocol("/v1/responses"); got != services.UpstreamProtocolOpenAIResponses {
		t.Fatalf("/v1/responses 期望 %s，实际 %s", services.UpstreamProtocolOpenAIResponses, got)
	}
	if got := services.DetectUpstreamProtocol("/responses/compact"); got != services.UpstreamProtocolOpenAIResponses {
		t.Fatalf("/responses/compact 期望 %s，实际 %s", services.UpstreamProtocolOpenAIResponses, got)
	}
}

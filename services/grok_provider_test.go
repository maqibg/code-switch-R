...
func TestValidateGrokProviderConfiguration(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "wildcard target", provider: Provider{ModelMapping: map[string]string{"grok-build": "grok-*"}}, contains: "不含通配符"},
		{name: "extra inbound", provider: Provider{ModelMapping: map[string]string{"grok-build": "grok-4.5", "other": "other"}}, contains: "只允许一个"},
	}
...
func TestHasEligibleGrokRelayProvider(t *testing.T) {
	...
	for name, provider := range map[string]Provider{
		"disabled":      {Name: "disabled", APIURL: valid.APIURL, APIKey: valid.APIKey, ModelMapping: valid.ModelMapping},
		"missing model": {Name: "missing model", APIURL: valid.APIURL, APIKey: valid.APIKey, Enabled: true},
		"missing key":   {Name: "missing key", APIURL: valid.APIURL, Enabled: true, ModelMapping: valid.ModelMapping},
	} {

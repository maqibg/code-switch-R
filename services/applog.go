...
// consoleLogSink 接收日志的控制台服务。用接口避免 applog 反向依赖具体实现。
type consoleLogSink interface {
	addLog(level, message string)
}

var (
	appLogSink    atomic.Pointer[consoleLogSink]
...
// ConsoleService 构造时调用。
func registerConsoleLogSink(sink consoleLogSink) {

...
func codexToolToChatTool(item any, index int) (map[string]any, error) {
	...
	if !ok {
		return nil, NewClientRequestRejectedError(fmt.Sprintf("tools[%d] 必须是对象", index))
	}
	if stringFromMap(obj, "type") != "function" {
		return nil, NewClientRequestRejectedError(fmt.Sprintf("tools[%d].type=%q 暂不支持", index, stringFromMap(obj, "type")))
	}
	name := stringFromMap(obj, "name")
	if strings.TrimSpace(name) == "" {
		return nil, NewClientRequestRejectedError(fmt.Sprintf("tools[%d].name 不能为空", index))
	}
...
func codexToolChoiceToChat(value any) (any, bool, error) {
	...
	if !ok {
		return nil, false, NewClientRequestRejectedError("tool_choice 必须是 string 或对象")
	}
...
func codexStringToolChoiceToChat(text string) (any, bool, error) {
	...
	default:
		return nil, false, NewClientRequestRejectedError(fmt.Sprintf("tool_choice=%q 暂不支持", text))
	}
}

func codexObjectToolChoiceToChat(obj map[string]any) (any, bool, error) {
	if stringFromMap(obj, "type") != "function" {
		return nil, false, NewClientRequestRejectedError(fmt.Sprintf("tool_choice.type=%q 暂不支持", stringFromMap(obj, "type")))
	}
	name := stringFromMap(obj, "name")
	if name == "" {
	...
	if name == "" {
		return nil, false, NewClientRequestRejectedError("tool_choice function name 不能为空")
	}

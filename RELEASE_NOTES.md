...
## 更新亮点
- 模型映射中已保存的规则支持点击就地修改：可直接编辑 CLI 模型与供应商模型，回车保存、Esc 取消，改 key 时自动替换旧键，不必再删除后重建。

## 修复
- 修复模型获取弹窗搜索框图标错位：该弹窗此前复用了别处的输入框类名却没有自己的样式，输入框退化成浏览器默认外观，写死像素定位的搜索图标随之对不齐；现由弹窗自身提供输入框样式，图标改为垂直居中。
...
## 更新亮点
- 移除供应商「模型白名单」机制，改用统一的显式模型映射决定请求路由：未配置映射时不再限制模型，配置映射后按精确或通配符匹配。模型发现能力接入供应商编辑器的模型映射与 Grok 上游模型选择。
- 全新模型获取弹窗：支持 OpenAI 兼容（`/v1/models`）与原生两种来源，可批量勾选导入、按提示删除已失效模型，并按供应商原始字段查找。

## 修复
- 修复 Grok Build 上游模型映射校验逻辑，避免因白名单约束导致合法配置无法保存。
...
## 更新亮点
- 日志页金额统一显示 `$`，时间改为日期和时间分两行，表头居中，日志内容支持选择复制。
- 发布构建自动下载并校验最新模型价格表，确保新版本内置价格数据保持更新。

## 修复
- 修复价格快照保存时内容摘要失效，避免升级后反复提示价格表无效。
- 修复无效本地价格快照重复告警，启动时会隔离坏快照并使用内置价格表。

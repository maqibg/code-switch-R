...
## 修复
- 修复从旧版本升级后 `request_log` 缺少凭据日志字段，导致请求日志写入失败、待重放日志持续失败的问题。
- 修复客户端已断开时上游请求返回 `context canceled` 仍被误记为 Provider 失败并触发降级的问题。
...
## 更新亮点
- 完成历史金额结构收口：数据库标准金额列统一为非空 `TEXT`，直接升级旧版本即可完成转换。
- 启动阶段同步清洗历史金额，非法字段按列归零并标记 `migration_zero`，非法总额按分项重算，合法不一致总额保留并在控制台告警。
- 日志和统计接口及页面统一最多显示 6 位小数并四舍五入去掉末尾 0，数据库和内部计算继续保留完整精度。

## 修复
- 删除旧的 `*_decimal` 临时列、迁移状态表和后台金额回填循环，避免升级后继续累积兼容代码。
- 修复请求日志和 Relay attempt 在标准金额列切换后的写入、统计聚合与展示精度问题。
...
## 修复
- 兼容仍在运行的旧版 Codex/CLI 进程：升级到持久化随机 Relay Token 后，旧进程缓存的历史固定凭据不会再被错误地直接拒绝。
- Relay 拒绝认证时写入不含凭据原文的控制台告警，便于区分本地认证失败与上游 401。
...
## 修复
- 修复从旧版本升级时 Codex `auth.json` 缺失导致应用在窗口创建前直接退出的问题；应用会恢复本地 Relay 认证文件并继续启动。
- 托管 CLI 凭据迁移改为逐平台执行；单个平台配置冲突只产生可见告警，不再阻塞其他平台和主界面启动。
- 增加旧版托管状态、认证文件缺失和跨平台迁移继续执行的回归测试。
- 发布构建从 tag 同步 Wails 文件版本，避免便携 exe 的文件版本落回默认 `0.1.0`。

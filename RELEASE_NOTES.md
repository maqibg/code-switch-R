...
## 更新亮点
- 计费金额改为精确保存和计算，避免浮点误差影响价格、预算、日志和统计结果。
- 新增历史日志金额的后台分批迁移、进度记录和有限备份，升级过程中可恢复且不会阻塞应用启动。
- 统一前后端金额接口为字符串，模型定价、请求成本、排行榜、热力图和托盘统计使用同一套精确计算规则。

## 修复
- 修复请求日志写入字段数量不匹配导致部分请求记录失败的问题。
- 规范 Pi 免费模型的零价格处理，恢复模型排行榜的金额排序。
